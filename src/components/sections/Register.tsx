  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Race condition: cap may have just been reached — refetch status so UI swaps.
      ...
      <div className="relative mx-auto max-w-5xl px-4 sm:px-6">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="font-display text-4xl text-foreground sm:text-5xl">
            {isClosed ? (
              <>Registration <span className="text-gradient-gold">Closed!</span></>
            ) : (
        ...
        {isClosed ? (
          <motion.div
            ...
            className="mt-12 rounded-3xl glass-strong p-8 text-center sm:p-12"
          >
            <div className="mx-auto mb-5 inline-flex h-16 w-16 items-center justify-center rounded-full bg-gradient-gold text-primary-foreground shadow-gold">
              <CheckCircle2 className="h-8 w-8" />
            </div>
            <p className="text-lg text-foreground sm:text-xl">
              We have successfully reached our target of <span className="font-semibold text-gradient-gold">2,000 participants</span> for Kwara Kre8ives 2.0.
            </p>
            <p className="mt-3 text-base font-medium text-foreground/90">
              Registration is now closed.
            </p>
            <p className="mx-auto mt-5 max-w-2xl text-sm leading-relaxed text-muted-foreground sm:text-base">
              Thank you for the overwhelming interest and support. We look forward to welcoming all registered participants to an inspiring day of learning, networking, and creative empowerment.
            </p>
          </motion.div>
        ) : (
          <motion.form
